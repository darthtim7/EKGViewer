...
# Clean application executes from the package directory, so resolve the baseline before cwd changes.
text = text.replace(
    'str(baseline_restore), "--output-dir", str(output)',
    'str(baseline_restore.resolve()), "--output-dir", str(output)',
)
...
text = text.replace('"RECOVERY_EVENTS_232_240.json"', '"RECOVERY_EVENTS_232_242.json"')
